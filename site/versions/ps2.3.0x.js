...
{
	"use strict";
	var id, len, i, rec, func, format, img;
	...
	img = PS.ImageData(this, format); // extract the data
	if ( img !== PS.ERROR )
	{
		try
		{
			func(img); // call user function with data
		}
		catch (err)
		{
...
PS.ImageLoad = function ( file, func, format )
{
	...
	// save a record with the user function and alpha preference
	
	id = "L" + PS.LoaderCnt; // a unique ID
	PS.LoaderCnt += 1;	
	PS.LoaderList.push( { id: id, func: func, format: format } );
...
{
	"use strict";
	var fn, xmax, ymax, w, h, t, data, psize, wsize, rowptr, ptr, drawx, drawy, ctx, bsize, cw, ch, y, x, r, g, b, a, rgb, rval, gval, i, bead, bs, bx, gx, by, gy, bw;
	...
	drawy = ypos; // y-pos of bead being drawn
	for ( y = 0; y < height; y += 1 )
	{
		// done if row is off bottom of grid
		...
		ptr = rowptr; // set pointer to start of row
		
		if ( drawy >= 0 )
		{		
			drawx = xpos; // x-pos of bead being drawn
			for ( x = 0; x < width; x += 1 )
			{
				// is this bead on grid?
				...
				if ( (drawx >= 0) && (drawx < xmax) )
				{
					...
					if ( a > 0 ) // do nothing if alpha is zero
					{
						...
						if ( bead.visible )
						{
							...
							{
								ctx.fillStyle = bead.glyphColor;
								ctx.fillText (bead.glyphStr, PS.Grid.glyphX + gx, PS.Grid.glyphY + gy);
							}
						}
						else
						{
	...
	PS.Grid.context.drawImage(PS.BlitCanvas, xpos, ypos, cw, ch, 0, 0, cw, ch);
...
PS.LoadHTML5Audio = function (fullpath, volume, func, now)
{
	...
	// find any unused or idle channel to put sound
		
	for ( i = 0; i < PS.AUDIO_MAX_CHANNELS; i += 1 )
	{
		channel = PS.AudioChannels[i];
		if ( (channel.status === PS.CHANNEL_EMPTY) || (channel.status === PS.CHANNEL_READY) )
		{			
			channel.id = fullpath;
