...
( function ()
{
	...
	var _KEY_RATE = 6; // key repeat rate (1/10 sec)
	...
	*/
	...
	// Return a color that is x% between c1 and c2

	function _dissolve ( c1, c2, x )
	{
		var delta;
...
//		_reportTime();
		...
		if ( len > 0 )
		{
			...
			{
				_keyDelay = _KEY_RATE; // reset delay
				for  ( i = 0; i < len; i += 1 )
				{
					key = _holding[ i ];
					try
	...
	function _resetBead ( bead )
	{
		...
		bead.planes = [ { height : 0, color : color } ]; // init planes array
...
//		PS.debug( "_touchStart finger = " + _currentFinger + "\n" );
	...
	// _legalKey ( event )
	// Returns true if a key should be acknowledged, else false

	function _legalKey ( key )
	{
		return ( ( key === 9 ) || ( key === 27 ) || ( key >= 32 ) );
	}

	// _keyFilter ( key, shift )
	// Translates weird keycodes to useful values
	...
	function _keyFilter ( key, shift )
	{
		...
		// This should just be an array lookup
		...
		switch ( key )
		{
			...
			case 36:
				...
				break;
			case 37:
				key = PS.KEY_ARROW_LEFT;
				break;
			case 38:
				key = PS.KEY_ARROW_UP;
				break;
			case 39:
				key = PS.KEY_ARROW_RIGHT;
				break;
			case 40:
				key = PS.KEY_ARROW_DOWN;
				break;
			case 45:
				key = PS.KEY_INSERT;
				break;
		...
		if ( PS.keyDown )
		{
			...
			if ( _legalKey( key ) )
			{
				...
				if ( !_pressed[ key ] )
				{
					...
					{
						_keyDelay = _KEY_RATE * 5;	// set initial repeat delay if no other keys down
					}
		...
		if ( PS.keyUp )
		{
			...
			if ( _legalKey( key ) )
			{
				...
				if ( PS.keyUp )
				{
					try
					{
						PS.keyUp( key, event.shiftKey, event.ctrlKey, _EMPTY );
						_gridDraw();
					}
					catch( err )
					{
	...
	function _collisionCheck ( s, id )
	{
		...
		for ( i = 0; i < len; i += 1 )
		{
			...
			if ( ( id2 !== id ) && s2.visible && s2.placed )
			{
				...
				// If dx/dy are EQUAL to the width/height of either sprite, they are touching

				if ( ( ( dx === w ) && ( dy === h ) ) || ( ( dx === w2 ) && ( dy === h2 ) ) )
				{
					evt = PS.SPRITE_TOUCH;
				}

				// If dx/dy are LESS THAN OR EQUAL TO to the width/height of either sprite, they are overlapping
				// We already know they're not both equal, so this test will work

				else if ( ( ( dx <= w ) && ( dy <= h ) ) || ( ( dx <= w2 ) && ( dy <= h2 ) ) )
				{
...
//			status.readonly = "readonly";
		...
		active : function ( x, y, active_p )
		{
			...
			if ( ( active !== PS.CURRENT ) && ( active !== true ) && ( active !== false ) )
			{
				...
				{
					if ( !active ) // this will catch the number zero
					{
						active = false;
					}
					else
...
//			PS.debug(fn + "id = " + id + "\n");
		...
		imageLoad : function ( filename_p, exec_p, format_p )
		{
			...
			try
			{
				...
				image.onload = function () { _imageLoad( image ); };
				image.onerror = function () { _imageError( image ); };
