		...
		if ( PS.UserDelay >= PS.UserClock )
		{
			...
			if ( PS.Tick )
			{
				...
				{					
					...
					PS.Oops("PS.Tick() failed [" + err2.message + "]" );
				}				
			}
		}
	}
...
PS.ImageBlit = function ( imgdata, xpos, ypos, left, top, width, height )
{
	...
	ctx.font = PS.Grid.glyphSize; // reset glyph size?
...
PS.AudioError = function (event)
{
	...
	return PS.Oops("HTML5 audio error " + c + " (" + str + ")");	
};
...
PS.AudioPath = function (path)
{
	...
	{
		return PS.Oops(fn + "parameter not a valid string");
	}
