// ps3.2.5.js for Perlenspiel 3.2
...
( function ()
{
	...
	var _MAX_GRID_SIZE = 512; // maximum client size in pixels
	var _MAX_FONT_SIZE = 1.25; // maximum font size in ems
	var _MAX_SHADOW_SIZE = 8; // max grid shadow size in pixels
	var _MAX_BLUR_SIZE = 64; // maximum grid shadow blur in pixels
	...
	var _graph = null; // graph DOM element
	var _graphing = null; // id of database if graph is visible
	...
	var _refreshed = 0; // number of beads refreshed
	var _seed = 1; // random number seed
	var _swiping = null; // collects current swipe data
	...
	var _tickCount; // clock tick counter
	...
	var _holdAlt; // true if alt key is held down
	...
	var _windowWidth; // width of browser window
	var _windowHeight; // height of browser window
	var _gridMax; // max width/height of grid
	var _fontMax; // max size of status line font
	var _touchDown = false; // true when mouse/finger is pressed
	...
	var _monitor; // monitor DOM element
	...
	var _keyDownOptions = { // static for efficiency
		time : 0
	};
			...
			else
			{
				...
				for ( i = 0; i < len; i += 1 )
				{
					...
					if ( key )
					{
						...
						{
							refresh = true;
							_keyDownOptions.time = _tickCount;
							PS.keyDown( key, _holdShift, _holdCtrl, _keyDownOptions );
						}
						catch ( e5 )
						{
...
//		bead.timeExit = 0;
	...
	function _addSwipe ( x, y, time )
	{
		_swiping.events.push( {
			x : x,
			y : y,
			start : time,
			end : 0,
	...
	function _updateSwipe ( time )
	{
		var len, event;

		len = _swiping.events.length;
		if ( len > 0 )
		{
			event = _swiping.events[ len - 1 ]; // get this event's swipe record
			_swiping.end = time;
			event.end = time; // set end time
			event.duration = ( time - event.start ); // update total duration
			_swiping.duration += event.duration;
		}
	}
	...
	function _copySwipe ()
	{
		...
		obj = {
			start : _swiping.start,
			end : _swiping.end,
			duration : _swiping.duration,
			events : []
		};
	...
	function _emitSwipe ( time )
	{
		var any, swipe;

		any = false;
		if ( _swiping.events.length > 1 ) // At least 1 event in swipe?
		{
			_updateSwipe( time );
			...
			if ( PS.swipe )
			{
				try {
					swipe = _copySwipe();
					PS.swipe( swipe, { time : time } );
					any = true;
				}
				catch ( err ) {
					return _errorCatch( "PS.swipe() failed [" + err.message + "]", err );
				}
			}
		}
		_swiping = null; // nuke global swipe record
		return any;
	}
	...
	var _touchBeadOptions = { // static for efficiency
		time : 0
	};
	...
	function _touchBead ( bead )
	{
		...
		{
			any = false;
			time = _tickCount; // get this only once
			bead.timeTouch = time;
			_touchBeadOptions.time = time;

			// Touch always starts a new swipe

			_initSwipe( time );
			_addSwipe( bead.x, bead.y, time );
			...
			if ( bead.exec )
			{
				...
				{
					data = _getData( bead );
					bead.exec( bead.x, bead.y, data, _touchBeadOptions );
					any = true;
				}
				catch ( e1 )
				{
			...
			if ( PS.touch )
			{
				try
				{
					data = _getData( bead ); // in case previous call changed it
					PS.touch( bead.x, bead.y, data, _touchBeadOptions );
					any = true;
				}
				catch ( e2 )
				{
	...
	var _releaseBeadOptions = { // static for efficiency
		time : 0
	};
	...
	function _releaseBead ( bead )
	{
		...
		{
			any = false;
			time = _tickCount; // get only once
			_releaseBeadOptions.time = bead.timeRelease = time;
			if ( PS.release )
			{
				try
				{
					data = _getData( bead );
					PS.release( bead.x, bead.y, data, _releaseBeadOptions );
					any = true;
				}
				catch ( e1 )
				{
					return _errorCatch( "PS.release() failed [" + e1.message + "]", e1 );
				}
			}

			// Release always ends a swipe

			if ( _emitSwipe( time ) || any )
			{
				_gridDraw();
			}
		}
	...
	var _enterBeadOptions = { // static for efficiency
		time : 0
	};
	...
	function _enterBead ( bead )
	{
		...
		{
			if ( PS.enter ) {
				time = _tickCount; // get this once
				_enterBeadOptions.time = bead.timeEnter = time;

				// If mouse/finger is down, register swipe event

				if ( _touchDown && _swiping )
				{
					_addSwipe( bead.x, bead.y, time );
				}
				try
				{
					data = _getData( bead );
					PS.enter( bead.x, bead.y, data, _enterBeadOptions );
					_gridDraw();
				}
				catch ( err )
				{
	...
	var _exitBeadOptions = { // static for efficiency
		time : 0
	};
	...
	function _exitBead ( bead )
	{
		...
		if ( bead.active )
		{
			...
			{
				time = _tickCount;
				_exitBeadOptions.time = bead.timeExit = time;
				if ( _touchDown && _swiping )
				{
					_updateSwipe( time );
				}
				...
				{
					data = _getData( bead );
					PS.exit( bead.x, bead.y, data, _exitBeadOptions );
					_gridDraw();
				}
				catch ( err )
				{
	...
	var _exitGridOptions = { // static for efficiency
		time : 0
	};
	...
	function _exitGrid ()
	{
		...
		{
			time = _tickCount;
			_exitGridOptions.time = _grid.timeExit = time;
			try {
				PS.exitGrid( _exitGridOptions );
				_gridDraw();
			}
			catch ( err ) {
			...
			if ( _touchDown && _swiping ) // At least 1 bead in swipe?
			{
				_addSwipe( -1, -1, time ); // add offgrid event
			}
		}
		return PS.DONE;
	}
...
//		PS.debug("_docDown()\n");
	...
	function _docUp ( event )
	{
//		PS.debug("_docUp()\n");
		if ( _swiping  )
		{
			_emitSwipe( _tickCount );
		}
		_touchDown = false;
		return _endEvent( event );
	}
...
//		PS.debug("_bugUp()\n");
	...
	function _mouseDown ( event )
	{
		...
		x = event.clientX; // now recognized by all browsers!
		y = event.clientY;
		_touchDown = true;
		bead = _getBead( x, y );
		if ( bead )
...
//				", s = " + _holdShift + ", c = " + _holdCtrl + ", a = " + _holdAlt + "\n");
			...
			else if ( key === _KEY_SHIFT )
			{
				...
				for ( i = 0; i < len; i += 1 )
				{
					...
					if ( key !== hardkey ) // if they differ
					{
						...
						{
							_keyDownOptions.time = _tickCount;
							PS.keyDown( key, true, _holdCtrl, _keyDownOptions );
							any = true;
						}
						catch ( err2 )
						{
	...
	var _keyUpOptions = { // static for efficiency
		time : 0
	};
...
//				", s = " + _holdShift + ", c = " + _holdCtrl + ", a = " + _holdAlt + "\n");
			...
			if ( _legalKey( key ) )
			{
				...
				try
				{
					_keyUpOptions.time = _tickCount;
					PS.keyUp( key, shift, ctrl, _keyUpOptions );
					any = true;
				}
				catch ( err )
				{
			...
			else if ( key === _KEY_SHIFT )
			{
				...
				for ( i = 0; i < len; i += 1 )
				{
					...
					if ( key !== hardkey ) // if they differ
					{
						...
						{
							_keyUpOptions.time = _tickCount;
							PS.keyDown( key, false, ctrl, _keyUpOptions );
							any = true;
						}
						catch ( err2 )
						{
	...
	var _wheelData = { // static for efficiency
		wheel : 0
	};

	var _wheelOptions = { // static for efficiency
		time : 0
	};
	...
	function _wheel ( event )
	{
		...
		if ( PS.input ) // only if function exists
		{
			...
			try
			{
				_wheelData.wheel = delta;
				_wheelOptions.time = _tickCount;
				PS.input( _wheelData, _wheelOptions );
				_gridDraw();
			}
			catch ( err )
			{
	...
	function _gridActivate ()
	{
		...
		document.addEventListener( "mousedown", _docDown, false );
		document.addEventListener( "mouseup", _docUp, false );
	...
	function _gridSize ( x, y )
	{
		...
		if ( !_grid.ready || ( x !== _grid.x ) || ( y !== _grid.y ) )
		{
			...
			if ( x >= y )
			{
				size = Math.floor( _gridMax / x );
			}
			else
			{
				size = Math.floor( _gridMax / y );
			}
...
//				else if ( ( bead.radius > 0 ) || ( bead.glyph.code > 0 ) )
	...
	{
		var id, bead, current, def, fader, rgb, r, g, b, a;

		id = ( y * _grid.x ) + x;
		bead = _beads[ id ];
		current = bead.border.color;
		def = _DEFAULTS.bead.border.color;
	...
	function _glyphColor ( x, y, colors )
	{
		...
		def = _DEFAULTS.bead.glyph.color;
	...
	var _db = {
		...
		validID : function ( id )
		{
			return _db.store.hasOwnProperty( id );
		},
		...
		{
			return _db.users.hasOwnProperty( uname );
		},
		...
		csv : function ( fn, id ) {
			...
			if ( rows < 1 ) {
				return _error( fn + "Database '" + id + "' is empty" );
			}
			cols = data.fields.length; // number of columns/fields
			max = _db.expand( data.tmax ); // expand maximum timestamp for formatting
			...
			for ( i = 0; i < rows; i += 1 ) {
				...
				str += _db.tstring( record[ _db.TIME_LABEL ], max );
				str += ",";
				for ( j = 0; j < cols; j += 1 ) {
					field = data.fields[ j ];
		...
		send : function ( fn, id, uname ) {
			...
			if ( !request )
			{
				return _error( fn + "XMLHttpRequest init failed\n" );
			}

			oline = PS.statusText(); // save status line text
			PS.statusText( "Sending data to " + uname + " ..." );
			...
			{
				if ( monitor ) {
					PS.debug( "XMLHttpRequest status: " + request.status + "\nText: " + request.responseText + "\n" );
				}
				PS.statusText( oline );
			};
			...
			{
				_error( fn + "XMLHttpRequest status: " + request.status + ", text: " + request.responseText );
				PS.statusText( oline );
			};
					...
					attachments: [
						{
							type : "text/plain",
							name : filename + ".csv",
							content : _db.b64Encode( _db.csv( fn, id ) )
						}
					]
				}
			});
			...
			{
				PS.statusText( oline );
				return _errorCatch( fn + "XMLHttpRequest send failed [" + err.message + "]" );
			}
	...
	function _sizeDetect ()
	{
		...
		min = ( _MAX_GRID_SIZE / 4 ) * 5; // minimum width/height
		if ( ( _windowWidth >= min ) && ( _windowHeight >= min ) )
		{
			max = _MAX_GRID_SIZE;
			fmax = _MAX_FONT_SIZE;
			smax = _MAX_SHADOW_SIZE;
			bmax = _MAX_BLUR_SIZE;
		}
		else
		{
			...
			fmax = _MAX_FONT_SIZE * ( max / _MAX_GRID_SIZE ); // calc percentage
			fmax = Math.floor( fmax * 100 ) / 100; // round to nearest 100th
			bmax = _MAX_BLUR_SIZE * ( max / _MAX_GRID_SIZE ); // calc percentage
			bmax = Math.floor( bmax );
			smax = Math.floor( bmax / 8 );
		}
		_gridMax = max;
		_fontMax = fmax + "em";
		_DEFAULTS.grid.shadow.params =  "0px 0px " + bmax + "px " + smax + "px ";
	}
	...
	PS = {
		...
		_sys: function () {
			var fn, i, outer, debug, sp, snode, ip, inode, span, input, grid, footer, ctx, cnt, bead, aq, result, str;
			...
			_seed = Math.floor( Math.random() * 1000000 ) + 1; // random seed for PS.random()
			...
			_systemDetect();
			_sizeDetect(); // establishes _gridMax and _fontMax
			...
			_statusOut("Perlenspiel " + _system.major + "." + _system.minor);

			// Create graph div

			_graphing = null;
			_graph = document.createElement( "div" );
			_graph.style.display = "none";
			_graph.style.marginBottom = "1.0em";
			_main.appendChild( _graph );
			...
			/*
			 ...
			 var renderer = PIXI.autoDetectRenderer( _gridMax, _gridMax );
			...
			_monitor = document.createElement("textarea");
			_monitor.id = _MONITOR_ID;
			_monitor.rows = 8;
			_monitor.wrap = "soft";
			_monitor.readonly = "readonly";
			_monitor.onfocus = function () {
				_debugFocus = true;
			};
			_monitor.onblur = function () {
				_debugFocus = false;
			};
			_monitor.addEventListener( "mousedown", _bugDown, false );
			_monitor.addEventListener( "mouseup", _bugUp, false );
			debug.appendChild(_monitor);
			...
			/*
			 _transKeys[ 12 ] = PS.KEY_PAD_5;
			 _transKeys[ 33 ] = PS.KEY_PAD_9;
			 _transKeys[ 34 ] = PS.KEY_PAD_3;
			 _transKeys[ 35 ] = PS.KEY_PAD_1;
			 _transKeys[ 36 ] = PS.KEY_PAD_7;
			 _transKeys[ 37 ] = PS.KEY_PAD_4;
			 _transKeys[ 38 ] = PS.KEY_PAD_8;
			 _transKeys[ 39 ] = PS.KEY_PAD_6;
			 _transKeys[ 40 ] = PS.KEY_PAD_2;
			 _transKeys[ 45 ] = PS.KEY_PAD_0;
			 _transKeys[ 46 ] = PS.KEY_DELETE;
			 _transKeys[ 110 ] = PS.KEY_DELETE;
			 */
			...
			if (typeof PS.swipe !== "function") {
				PS.swipe = null;
				_warning("PS.swipe" + str);
			}
			...
			if (PS.init) {
				...
				_system.date = _getDate();

				try {
					PS.init(_system, { time : _tickCount } );
					_gridDraw();
				}
				catch (err) {
		...
		statusColor: function (p1, p2, p3) {
			var fn, colors, current, def, fader, rgb, r, g, b;
...
//			PS.imageDump( s.image );
		...
		dbInit : function ( id ) {
			...
			id = id.toLowerCase();
			if ( id === _db.TIME_LABEL ) {
				return _error( fn + "Database id '" + _db.TIME_LABEL + "' is reserved" );
			}
			if ( _db.validID( id ) ) {
				return _error( fn + "Database id '" + id + "' already exists" );
			}
			_db.init( id );
			return id;
		},
		...
		dbErase : function ( id ) {
			...
			id = id.toLowerCase();
			if ( !_db.validID( id ) ) {
				return _error( fn + "Database id '" + id + "' does not exist" );
			}
			_db.erase( id );
			return id;
		},
		...
		dbEvent : function ( id ) {
			...
			args = arguments.length;
			if ( args < 3 ) {
				return _error( fn + "Missing arguments" );
			}
			if ( ( typeof id !== "string" ) || ( id.length < 1 ) ) {
				return _error( fn + "Database id invalid" );
			}
			...
			while ( i < args ) {
				...
				if ( ( typeof name !== "string" ) || ( name.length < 1 ) ) {
					return _error( fn + "Event name @ argument " + i + " invalid" );
				}
				name = name.toLowerCase();
				if ( name === _db.TIME_LABEL ) {
					return _error( fn + "Event label '" + _db.TIME_LABEL + "' is reserved" );
				}
				if ( nlist.hasOwnProperty( name ) ) {
					return _error( fn + "Duplicate event '" + name + "' @ argument " + i );
				}
				nlist[ name ] = true; // remember this event name
				nxt = i + 1;
				if ( nxt >= args ) {
					return _error( fn + "Missing value for event '" + name + "'" );
				}
				elist.push( name );
				val = arguments[ nxt ];
				type = _typeOf( val );
				if ( ( type !== "number" ) && ( type !== "string" ) && ( val !== true ) &&
					( val !== false ) && ( val !== null ) ) {
					return _error( fn + "Illegal value type for event '" + name + "'" );
				}
				elist.push( val );
				i += 2;
			}

			return _db.event( id, elist );
		},
		...
		dbShow : function ( id, options ) {
			...
			if ( args === 2 )
			{
				...
				{
					return _error( fn + "Invalid options parameter" );
				}
			}
			else
			{
			...
			try
			{
				dg = new Dygraph( _graph, str, opt );
			}
			catch ( err )
			{
				_graph.style.display = "none";
				return _errorCatch( fn + "Dygraph display error [" + err.message + "]", err );
			}
			_graphing = id;
			return id;
		},
		...
		dbSend : function ( id, uname ) {
			...
			if ( ( typeof uname !== "string" ) || ( uname.length < 1 ) ) {
				return _error( fn + "Username invalid" );
			}
			uname = uname.toLowerCase();
			if ( !_db.validUser( uname ) ) {
				return _error( fn + "Username '" + id + "' not recognized" );
			}

			return _db.send( fn, id, uname );
		}
	};
}() );
